/*
  # Fix Replicate Generations Policy

  1. Changes
    - Drops the existing policy "Allow authenticated users to insert generations" if it exists
    - Recreates the policy with the same name but ensures it doesn't conflict
  
  2. Security
    - Maintains the same security model with RLS enabled
*/
